...
- センサを **ドラッグで移動 / 空白クリックで追加 / Shift+クリックで半径変更・削除** することで配置を編集し、「再計算」で 100〜300 ms 程度で新しい BDD を組み直し
...
webapp は親ディレクトリの `pgcp-experiments/` を **必要としません**（ソルバとアナリシスは `webapp/server/` に同梱）。
このディレクトリ単体をコピーすれば任意の場所で動かせます。
...
|---|---|
| センサをクリック | 故障/復旧トグル |
| Shift + センサをクリック | ポップアップ表示（半径スライダ・削除） |
| センサをドラッグ | 位置移動 |
| 地図の空白をクリック | 新規センサ追加（半径は側パネル設定） |
...
- Shift+クリックで開くポップアップ内のクリックはマップに伝播しないようガード済み。万一誤って空白クリック扱いになっても `.leaflet-interactive` 判定で再防御しています
